...
cpl::utils::Cpl_ForceOptimization::Cpl_ForceOptimization(XBot::ModelInterface::Ptr model,
    ...
    _contact_links(contact_links)
{
    ...
    _min_wrench_aggr = boost::make_shared<OpenSoT::tasks::force::Wrenches>("_min_wrench_aggr",
                                                                          _contact_links,
                                                                          _contact_links,
                                                                          _wrenches);
...
void cpl::utils::Cpl_ForceOptimization::setContactRotationMatrix(const std::string& contact_link,
...
void cpl::utils::Cpl_ForceOptimization::setReferenceContactForce(const std::string& contact_link,
                                                                 const Eigen::Vector3d& Fc_ref)
{
    ...
    auto wrench_task = _min_wrench_aggr->getWrenchTask(contact_link);

    Eigen::Vector6d wrench_ref;
    wrench_ref.setZero();
    wrench_ref.head(3) = Fc_ref;

    wrench_task->setReference(wrench_ref);

    if(Fc_ref.norm() < 1e-12)
       wrench_task->setWeight(1e3*weight);
    else
       wrench_task->setWeight(weight);

}

bool cpl::utils::Cpl_ForceOptimization::compute(const Eigen::VectorXd& fixed_base_torque,
...
void cpl::utils::Cpl_ForceOptimization::log(XBot::MatLogger::Ptr logger)
{
