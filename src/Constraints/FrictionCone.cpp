...
void FrictionCone::FillJacobianBlock (std::string var_set, ifopt::Composite::Jacobian& jac_block) const 
{
    ...
    {     
        ...
        jac_block.coeffRef(1, 0) = (t2*(n.x()*n.x()-1.0)*2.0+n.x()*n.y()*t3*2.0+n.x()*n.z()*t4*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*n.x();
        jac_block.coeffRef(1, 1) = (t3*(n.y()*n.y()-1.0)*2.0+n.x()*n.y()*t2*2.0+n.y()*n.z()*t4*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*n.y();
        jac_block.coeffRef(1, 2) = (t4*(n.z()*n.z()-1.0)*2.0+n.x()*n.z()*t2*2.0+n.y()*n.z()*t3*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*n.z();
    ...
    {     
        ...
        jac_block.coeffRef(1, 0) = (t2*(t6+t7+t5*2.0)*2.0+F.x()*n.y()*t3*2.0+F.x()*n.z()*t4*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*F.x();
        jac_block.coeffRef(1, 1) = (t3*(t5+t7+t6*2.0)*2.0+F.y()*n.x()*t2*2.0+F.y()*n.z()*t4*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*F.y();
        jac_block.coeffRef(1, 2) = (t4*(t5+t6+t7*2.0)*2.0+F.z()*n.x()*t2*2.0+F.z()*n.y()*t3*2.0)*1.0/sqrt(t2*t2+t3*t3+t4*t4)*(-1.0/2.0) - _mu*F.z();
