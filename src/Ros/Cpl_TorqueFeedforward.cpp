...
#include <centroidal_planner/SetContactForce.h>
#include <centroidal_planner/SetManipWrench.h>

std::map<std::string, Eigen::Vector6d> * g_fmap_ptr;
std::map<std::string, ros::Time> * g_timeoutmap_ptr;
Eigen::Vector6d * manip_wrench_ptr;
const double FORCE_TTL = 0.1;
boost::shared_ptr<cpl::utils::Cpl_ForceOptimization> g_fopt;
...
bool on_manip_wrench_recv(centroidal_planner::SetManipWrenchRequest& req,
                          centroidal_planner::SetManipWrenchResponse& res)
{

    tf::wrenchMsgToEigen(req.manip_wrench.wrench, manip_wrench_ptr[0]);

    std::stringstream ss;
    Eigen::IOFormat print_fmt(2, 0, ", ", ";", "", "", "[", "]");
    ss <<  manip_wrench_ptr[0].format(print_fmt) << std::endl;

    res.success = true;
    res.message = "Successfully changed manipulation wrench to: "  + ss.str();
...
int main(int argc, char ** argv)
{
    ...
    ros::init(argc, argv, "cpl_torque_feedforward_node");
    ros::NodeHandle nh("cpl_torque_feedforward");
    ...
    /* Manipulation wrench */
    Eigen::Vector6d manip_wrench;
    manip_wrench.setZero();
    ...
    ros::ServiceServer contact_force_srv;
    ros::ServiceServer manip_wrench_srv;

    if(model->isFloatingBase())
    {
        auto f_opt = g_fopt = boost::make_shared<cpl::utils::Cpl_ForceOptimization>(model,
                                                                                    contact_links,
    ...
    while(ros::ok())
    {
        ...
        /* Add manipulation wrench */
        manip_wrench_ptr = &manip_wrench;
        tau.head(6) += manip_wrench;
